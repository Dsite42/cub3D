...
/*   Updated: 2023/09/05 17:20:33 by cgodecke         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double ray_Q1(t_data *data, int *i, int *j, int *wall)
{
	...
	{
		//data->ray_x = data->player_y - fabs(ray_x_length * sin(deg_to_rad(data->ray_angle)));
	...
	else
	{
			//if (data->ray_angle == 0)
			//	data->ray_angle = 0.01;
		//data->ray_x = data->player_y - fabs(1.0 * ray_y_length * sin(deg_to_rad(data->ray_angle)));
		//data->ray_y = data->player_x + fabs(1.0*ray_y_length * cos(deg_to_rad(data->ray_angle)));
		wall_row = (int)(floor(data->player_y - fabs(1.0 * ray_y_length * sin(deg_to_rad(data->ray_angle)))));
		wall_column = (int)(floor(data->player_x + fabs(1.0*ray_y_length * cos(deg_to_rad(data->ray_angle)))));
		*wall = map[wall_row][wall_column];
	...
	if (*wall == 1)
		printf("Q1 wall_row: %d, wall_column: %d distance: %f ray_angle: %f sky_direction: %d\n", wall_row, wall_column, distance, data->ray_angle, data->sky_direction);
	return (distance);
}
